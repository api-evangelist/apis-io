...
// A run buried in governance/<run>/<date>/ is a run nobody finds. This regenerates a
// stable, always-current index at governance/README.md on every run: what ruleset ran,
// what it found, and where each report and coverage payload lives.
const pct = (r) => `${r.coverage}%`;
const table = [
  '| API | Ops | Errors | Warnings | Info | Coverage | Score | Report | Data |',
  '| --- | --- | --- | --- | --- | --- | --- | --- | --- |',
  ...rows.map((r) => {
    const slug = manifest.find((m) => m.tag === r.tag).slug;
    return `| ${r.tag} | ${r.ops} | ${r.errors || '—'} | ${r.warnings} | ${r.info ?? 0} | ${pct(r)} | **${r.score} (${r.grade})** | [html](${runName}/${today}/${slug}.html) | [json](${runName}/${today}/${slug}.json) |`;
  }),
  `| **Total** | **${summary.totals.operations}** | **${summary.totals.errors}** | **${summary.totals.warnings}** | **${summary.totals.info}** | | ${summary.totals.passing}/${summary.totals.apis} passing | | [summary](${runName}/${today}/summary.json) |`,
].join('\n');

const readme = `# Governance
...
**Last run:** ${today} · **ruleset:** \`${summary.ruleset}\` · **via:** \`${API}\`
